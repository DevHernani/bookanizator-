...
const App = {
  ...
  bindEvents() {
    ...
    // Manual add button
    document.getElementById('add-manual-btn')?.addEventListener('click', () => this.openManualModal());
    document.getElementById('manual-close')?.addEventListener('click', () => this.closeManualModal());
    document.getElementById('manual-cancel')?.addEventListener('click', () => this.closeManualModal());
    document.getElementById('manual-book-form')?.addEventListener('submit', (e) => this.saveManualBook(e));

    // Manual cover button
    document.getElementById('manual-cover-btn')?.addEventListener('click', () => this.openManualCoverModal());
    document.getElementById('cover-close')?.addEventListener('click', () => this.closeManualCoverModal());
    document.getElementById('cover-cancel')?.addEventListener('click', () => this.closeManualCoverModal());
    document.getElementById('manual-cover-form')?.addEventListener('submit', (e) => this.saveManualCover(e));
    document.getElementById('cover-url')?.addEventListener('input', (e) => this.previewCover(e));
  },
  ...
  openManualModal() {
    const modal = document.getElementById('manual-book-modal');
    if (modal) {
  ...
  saveManualBook(event) {
    ...
    if (isbn !== 'N/A' && Storage.isbnExists(isbn)) {
      UI.showNotification('Este ISBN já está na biblioteca.', 'warning');
      return;
    }

    const savedBook = Storage.addBook(book);

    if (savedBook) {
      UI.showNotification('Livro adicionado com sucesso!', 'success');
      this.closeManualModal();
      this.refreshBooksList();
    } else {
      UI.showNotification('Erro ao adicionar o livro.', 'error');
    }
  },

  // Open manual cover modal
  openManualCoverModal() {
    const modal = document.getElementById('manual-cover-modal');
    if (modal) {
      ...
      if (select) {
        ...
        books.forEach(book => {
          ...
          option.textContent = `${book.title} - ${book.authors.join(', ')}`;
          select.appendChild(option);
        });
      }

      modal.classList.add('active');
      document.getElementById('manual-cover-form').reset();
      document.getElementById('cover-preview').innerHTML = '<p style="color: var(--text-secondary);">Cole a URL acima para ver o preview</p>';
    }
  },

  // Close manual cover modal
  closeManualCoverModal() {
    const modal = document.getElementById('manual-cover-modal');
    if (modal) {
      modal.classList.remove('active');
    }
  ...
  previewCover(event) {
    ...
    if (!url) {
      preview.innerHTML = '<p style="color: var(--text-secondary);">Cole a URL acima para ver o preview</p>';
      return;
    }

    preview.innerHTML = `<img src="${url}" alt="Preview" style="max-width: 200px; max-height: 300px; border-radius: 8px;" onerror="this.parentElement.innerHTML='<p style=\\'color: #ef4444;\\'>URL inválida ou imagem não encontrada</p>'">`;
  },

  // Save manual cover
  saveManualCover(event) {
    event.preventDefault();
    ...
    if (!bookId) {
      UI.showNotification('Por favor, selecione um livro.', 'warning');
      return;
    }

    if (!coverUrl) {
      UI.showNotification('Por favor, insira a URL da capa.', 'warning');
      return;
    }

    // Update book with new cover
    const updatedBook = Storage.updateBook(bookId, { cover: coverUrl });

    if (updatedBook) {
      UI.showNotification('Capa adicionada com sucesso!', 'success');
      this.closeManualCoverModal();
      this.refreshBooksList();
    } else {
      UI.showNotification('Erro ao adicionar a capa.', 'error');
    }
  }
};
